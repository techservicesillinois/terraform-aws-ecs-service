locals {
  ...
  lb_vpc_id     = "${element(concat(data.aws_lb.selected.*.vpc_id, list("")), 0)}"
  subnet_vpc_id = "${element(concat(data.aws_subnet.selected.*.vpc_id, list("")), 0)}"
}
...
data "aws_security_group" "lb" {
  count = "${length(var.load_balancer) > 0 ? 1 : 0}"
  name  = "${data.aws_lb.selected.name}"
}
