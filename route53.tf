...
locals {
  internal_lb = "${element(concat(data.aws_lb.selected.*.internal, list("")), 0)}"
}
...
resource "aws_route53_record" "default" {
  ...
  alias {
    name                   = "${element(concat(data.aws_lb.selected.*.dns_name, list("")), 0)}"
    zone_id                = "${element(concat(data.aws_lb.selected.*.zone_id, list("")), 0)}"
